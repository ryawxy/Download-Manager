		...
		if !q.creatingNewQueue {
			...
			case "up":
				if q.buttonsCursor != -1 {
					...
					q.contentCursor = (q.contentCursor + len(q.inputs) - 1) % len(q.inputs)
				}
			case "down":
				if q.buttonsCursor != -1 {
					q.buttonsCursor = -1
					...
					q.contentCursor = (q.contentCursor + 1) % len(q.inputs)
				}
			case "right":
				if q.buttonsCursor == -1 {
					q.controlContent = true
...
func (q *QueuesTab) getFooter() string {
	return "Press 'n' for New Queue | '→' to edit fields | '←' to go back | '↑/↓' to navigate | 'Enter' to select"
}
